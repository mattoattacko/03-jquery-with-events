...
articleView.populateFilters = function() { //pupulateFilters = a key, it has a value of "function"
  $('article').each(function() {
    // REVIEWED: We can declare several variables at once and assign their values later when using let. Keep in mind that we cannot do this with const. You can't change what the variable referrs to with const. 
    ...
    if (!$(this).hasClass('template')) {
      ...
      // TODONE, Build an <option> DOM element that we can append to the author <select> element.
...
articleView.handleAuthorFilter = function() {
  $('#author-filter').on('change', function() {
    ...
    if ($(this).val()) {  //Hey jQuery, I want to find the value of this. "If's" are looking for truthy or falsy. Empty string, undefined, and null are falsy. If you found something, then it's truthy. 
      ...
      // TODONE: If the <select> menu was changed to an option that has a truthy value, we first need to hide all the articles, and then show just the ones that match for the author that was selected.
